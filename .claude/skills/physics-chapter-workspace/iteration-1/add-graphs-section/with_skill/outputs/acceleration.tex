...
\begin{itemize}[itemsep=2pt,topsep=2pt]
  ...
  \item read the \emph{position--time}, \emph{velocity--time}, and \emph{acceleration--time} graphs of one motion together, passing from each to the next by slopes and areas or by derivatives and integrals, and recognise the shapes that constant acceleration produces.
\end{itemize}
\end{tcolorbox}
...
\elem Suppose the stone falls a distance $\Delta y_1$ in a time $t_1$, at which moment its velocity is $v_{y1}$. Let it keep falling until it has covered twice that distance, $2\Delta y_1$.

Hypothesis~\eqref{eq:hypA} says the velocity is now $2 v_{y1}$. What does the clock say? The stone has fallen through two segments, each of length $\Delta y_1$. Everyone agrees that the stone is moving faster in the second segment than in the first, so the second segment takes \emph{less} time than the first. The time to fall $2\Delta y_1$ is therefore some $t_2 < 2 t_1$. If we then let the stone keep falling until the clock reads $2t_1$, it gets faster still. So under hypothesis~\eqref{eq:hypA}, at time $2t_1$ the velocity is \emph{more} than $2 v_{y1}$.

Hypothesis~\eqref{eq:hypB} says that at time $2t_1$ the velocity is \emph{exactly} $2 v_{y1}$.

These are different predictions for the same measurement, so the hypotheses are genuinely different. An experiment that measured velocity at time $2t_1$ could tell them apart.

\calc Under \eqref{eq:hypB}, $v = gt$, so $v(2t_1) = 2g t_1 = 2v(t_1)$ exactly. Under \eqref{eq:hypA}, $\dd v/\dd t = kv$, whose solution (Worked Example~\ref{ex:exponential}) is $v(t) = v(t_1)\,e^{k(t - t_1)}$ once the stone is moving. Then $v(2t_1) = v(t_1)\,e^{k t_1}$, and since $e^{x} > 1 + x$ for $x > 0$, this exceeds $v(t_1)(1 + kt_1)$. Whether that exceeds $2v(t_1)$ depends on $kt_1$, but the point is that the ratio $v(2t_1)/v(t_1)$ is $e^{kt_1}$ under one hypothesis and exactly $2$ under the other; the two agree only by coincidence, for one special $t_1$, and disagree for every other. A single pair of speed measurements at $t_1$ and $2t_1$ distinguishes them.\qed
\end{example}
...
The free-fall acceleration is a property of the place, not of the falling object. Galileo's discovery, confirmed countless times since, is that when air resistance is small enough to ignore, \emph{all} objects at a given place fall with the same acceleration, whatever their mass, size, or material. A coin and a feather dropped in a vacuum tube land together. (Why this should be so is a question for the next chapters, where we meet force and mass.)
...
\begin{itemize}
  ...
  \item The three \textbf{motion graphs} of one motion, position--time, velocity--time, and acceleration--time, are linked. \elemtag\ Going down the stack takes slopes (the tangent slope of $x$--$t$ is $v_x$; the slope of $v_x$--$t$ is $a_x$); going up takes areas (under $a_x$--$t$ is $\Delta v_x$; under $v_x$--$t$ is $\Delta x$), negative below the axis. \calctag\ Differentiate going down, integrate going up; the sign of $a_x = \dd^2 x/\dd t^2$ sets which way the $x$--$t$ curve bends. Constant acceleration shows as a parabola, a straight line, and a horizontal line respectively. A graph records instrument readings against time; it is not a map of the path.
\end{itemize}
...
reading graphs & slope of $x$--$t$ is $v_x$; slope of $v_x$--$t$ is $a_x$ & $v_x = \dfrac{\dd x}{\dd t}$, \quad $a_x = \dfrac{\dd v_x}{\dd t}$ \\
 & area under $a_x$--$t$ is $\Delta v_x$; under $v_x$--$t$ is $\Delta x$ & $\Delta v_x = \displaystyle\int a_x\,\dd t$, \quad $\Delta x = \displaystyle\int v_x\,\dd t$ \\
...
free fall \textperiodcentered\ hypothesis \textperiodcentered\ proportionality \textperiodcentered\ instantaneous velocity \textperiodcentered\ derivative \textperiodcentered\ integral \textperiodcentered\ differential equation \textperiodcentered\ inclined plane \textperiodcentered\ motion diagram \textperiodcentered\ odd-number rule \textperiodcentered\ first difference \textperiodcentered\ second difference \textperiodcentered\ acceleration \textperiodcentered\ constant (uniform) acceleration \textperiodcentered\ square numbers \textperiodcentered\ telescoping sum \textperiodcentered\ velocity--time graph \textperiodcentered\ position--time graph \textperiodcentered\ acceleration--time graph \textperiodcentered\ tangent \textperiodcentered\ concave up, concave down \textperiodcentered\ area under the graph \textperiodcentered\ mean-speed theorem \textperiodcentered\ exponential growth \textperiodcentered\ free-fall acceleration $g$ \textperiodcentered\ constant-acceleration model \textperiodcentered\ deceleration \textperiodcentered\ hang time
...
\subsection*{Reading motion graphs}
\begin{enumerate}[label=\textbf{\thechapter.\arabic*},resume=exercises,leftmargin=*]
  \item Using only the velocity--time graph of Figure~\ref{fig:threegraphs}(b), find the tram's position at $t = \SI{4}{\second}$, \SI{14}{\second}, and \SI{22}{\second}, \elemtag\ from areas and \calctag\ from the piecewise formulas of Worked Example~\ref{ex:tramgraphs}. Check each against panel~(a).
  \item A lift rises from rest with acceleration \SI{1.2}{\metre\per\second\squared} for \SI{2.0}{\second}, travels at constant velocity for \SI{5.0}{\second}, then slows at \SI{1.2}{\metre\per\second\squared} to a stop in \SI{2.0}{\second}. Sketch its acceleration--time, velocity--time, and height--time graphs one above the other on a shared time axis, and find how far it rises. About how many storeys is that?
  \item Figure~\ref{fig:thrown} is the velocity--time graph of a ball thrown straight up at \SI{15}{\metre\per\second}. Sketch the matching height--time and acceleration--time graphs on the same time axis. At which instant is the slope of the height--time graph zero, and what are the velocity and the acceleration then? Does the acceleration--time graph do anything special at that instant?
  \item $\star$ A position--time graph consists of three straight segments: from $(\SI{0}{\second}, \SI{0}{\metre})$ to $(\SI{10}{\second}, \SI{50}{\metre})$, then horizontal to $(\SI{15}{\second}, \SI{50}{\metre})$, then straight down to $(\SI{20}{\second}, \SI{0}{\metre})$. (a) Describe the motion in words. (b) Sketch the velocity--time and acceleration--time graphs. (c) Find the displacement and the total distance travelled over the \SI{20}{\second}. (d) What does the graph imply about the acceleration at $t = \SI{10}{\second}$ and $t = \SI{15}{\second}$, and why can that not be literally true of a real object?
\end{enumerate}
...
\begin{description}[leftmargin=2em,itemsep=2pt]
  ...
  \item[2.8] Crossing position--time graphs: the cyclists are at the same place at that instant (one is passing the other), but their velocities, the slopes, can be quite different. Crossing velocity--time graphs: they have the same velocity at that instant, but they need not be anywhere near each other, since position is the accumulated area under each graph and depends on the whole history.
\end{description}
...
\begin{description}[leftmargin=2.6em,itemsep=3pt,style=sameline]
  ...
  \end{align*}
  \item[2.52] \SI{8}{\metre}, \SI{80}{\metre}, \SI{140}{\metre}. \elemtag\ Triangle $\tfrac12(4)(4.0) = 8$; $32 + (6)(8.0) = 80$; $128$ plus the trapezium $\tfrac12(8.0 + 4.0)(2) = 12$. \calctag\ $\tfrac12(1.0)(4)^2 = 8$; $32 + 8.0(6) = 80$; $128 + 8.0(2) - 2^2 = 140$.
  \item[2.53] Top speed $(1.2)(2.0) = \SI{2.4}{\metre\per\second}$. The acceleration graph is three horizontal steps, $+1.2$, $0$, $-1.2\ \si{\metre\per\second\squared}$; the velocity graph is a trapezium rising to \SI{2.4}{\metre\per\second}; the height graph bends up, runs straight, then bends over and levels off. Rise $= 2.4 + 12.0 + 2.4 = \SI{16.8}{\metre}$, about five storeys.
  \item[2.54] Height: the parabola $y = 15t - 4.9t^2$, peaking at \SI{11.5}{\metre} when $t = \SI{1.53}{\second}$ and returning to zero at \SI{3.06}{\second}. Acceleration: a horizontal line at \SI{-9.8}{\metre\per\second\squared} for the whole flight. The height graph has zero slope at $t = \SI{1.53}{\second}$, where $v_y = 0$ and $a_y = \SI{-9.8}{\metre\per\second\squared}$; the acceleration graph does nothing special there, which is the point of the Key Idea ``Acceleration is not velocity.''
  \item[2.55] (a) Forward at \SI{5}{\metre\per\second} for \SI{10}{\second}, at rest for \SI{5}{\second}, then back to the start at \SI{10}{\metre\per\second} in \SI{5}{\second}. (b) Velocity: steps at $+5$, $0$, $-10\ \si{\metre\per\second}$; acceleration: zero everywhere except downward spikes at \SI{10}{\second} and \SI{15}{\second}. (c) Displacement zero; distance \SI{100}{\metre}. (d) The velocity jumps instantly, so the acceleration would be infinite for zero time. A real object changes velocity over a short but nonzero interval, so the corners are really short steep curves and the spikes are large but finite.
  \item[2.56--2.59] Answers depend on your own measurements. Typical ramp results give second differences constant to within \SIrange{10}{20}{\percent}; typical phone-video drops give $g$ within a few percent of \SI{9.8}{\metre\per\second\squared} once several trials are averaged; typical hang times are \SIrange{0.4}{0.6}{\second}.
\end{description}

\section*{Sources and Further Reading}
\begin{itemize}[itemsep=3pt]
  \item P.~G. Hewitt, \emph{Conceptual Physics}, 12th ed. (Pearson, 2014). Chapter~2 on Galileo's inclined-plane experiments; Chapter~3, Sections 3.4 (``Acceleration'') and 3.5 (``Free Fall''), on the definition and units of acceleration, Galileo's inclined planes, free-fall speed and distance, objects thrown upward, and hang time; Appendix~C (``Graphing'') on reading the slope and the area under a velocity--time graph, and the area under an acceleration--time graph, which Section~\ref{sec:graphs} follows. Several Check Your Understanding questions and exercises, and the free-fall tables, are adapted from this book, with $g$ taken as \SI{9.8}{\metre\per\second\squared} rather than Hewitt's rounded \SI{10}{\metre\per\second\squared} except where noted. Hewitt's treatment is deliberately non-mathematical and corresponds to the \elemtag\ route throughout; the \calctag\ route is our addition, and every result here reduces to his when derivatives and integrals are read as slopes and areas.
