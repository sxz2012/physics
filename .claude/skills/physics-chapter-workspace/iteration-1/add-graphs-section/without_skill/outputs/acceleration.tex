...
\begin{itemize}[itemsep=2pt,topsep=2pt]
  ...
  \item read \emph{position--time}, \emph{velocity--time}, and \emph{acceleration--time} graphs of one motion and translate between them, taking slopes going down the stack and areas going up.
\end{itemize}
\end{tcolorbox}
...
\node[red!70!black,font=\small,anchor=south west] at (axis cs:5.45,1.78) {slope $= a_x$};
...
\node[teal!60!black,font=\small,align=center] at (axis cs:1,0.5) {area $= \Delta v_x$\\$= \SI{2}{\metre\per\second}$};
...
\begin{itemize}
  ...
  \item \textbf{Motion graphs.} Position--time, velocity--time, and acceleration--time graphs are three views of one motion. Going down the stack, each graph is the slope of the one above; going up, each is the area under the one below. A straight position--time graph means zero acceleration; one that bends upward means $a_x > 0$, downward means $a_x < 0$. \elemtag\ Slopes by ruler, areas by rectangles and triangles, bending by eye. \calctag\ $v_x = \dd x/\dd t$, $a_x = \dd^2 x/\dd t^2$, and the sign of the second derivative is the direction of bending. The position--time graph is smooth, the velocity--time graph may have corners, and the acceleration--time graph may jump.
\end{itemize}
...
reading graphs (down) & slope of $x$--$t$: $v_x$; \; slope of $v$--$t$: $a_x$ & $\dfrac{\dd x}{\dd t}$, \; $\dfrac{\dd v_x}{\dd t}$ \\
reading graphs (up) & area under $a$--$t$: $\Delta v_x$; \; under $v$--$t$: $\Delta x$ & $\displaystyle\int a_x\,\dd t$, \; $\displaystyle\int v_x\,\dd t$ \\
\bottomrule
\end{tabular}
\end{center}

\subsection*{Key terms}
free fall \textperiodcentered\ hypothesis \textperiodcentered\ proportionality \textperiodcentered\ instantaneous velocity \textperiodcentered\ derivative \textperiodcentered\ integral \textperiodcentered\ differential equation \textperiodcentered\ inclined plane \textperiodcentered\ motion diagram \textperiodcentered\ odd-number rule \textperiodcentered\ first difference \textperiodcentered\ second difference \textperiodcentered\ acceleration \textperiodcentered\ constant (uniform) acceleration \textperiodcentered\ square numbers \textperiodcentered\ telescoping sum \textperiodcentered\ position--time graph \textperiodcentered\ velocity--time graph \textperiodcentered\ acceleration--time graph \textperiodcentered\ area under the graph \textperiodcentered\ mean-speed theorem \textperiodcentered\ exponential growth \textperiodcentered\ free-fall acceleration $g$ \textperiodcentered\ constant-acceleration model \textperiodcentered\ deceleration \textperiodcentered\ hang time
...
\begin{description}[leftmargin=2em,itemsep=2pt]
  ...
  \item[2.8] The position--time graph is an arch: a parabola rising from $y = 0$ to a peak of \SI{11.5}{\metre} at $t = \SI{1.53}{\second}$ and returning to zero at \SI{3.06}{\second}. At the top of the flight its tangent is horizontal, since $v_y = 0$ there, but the graph is not flat for any length of time and keeps bending downward, since $a_y = -g$ throughout. The acceleration--time graph is a horizontal line at $\SI{-9.8}{\metre\per\second\squared}$. The area under it is $(-9.8)(3.06) = \SI{-30}{\metre\per\second}$, the change in velocity from $+15$ to $\SI{-15}{\metre\per\second}$ over the flight.
\end{description}
